	...
	if affinity.PodAntiAffinity != nil {
		...
		for _, v := range podAntiAffinityTerms {
			if v.TopologyKey != unversioned.LabelHostname {
				return apierrors.NewForbidden(a.GetResource(), pod.Name, fmt.Errorf("affinity.PodAntiAffinity has TopologyKey other than :%v is forbidden", unversioned.LabelHostname))
			}
		}
	}
	return nil
